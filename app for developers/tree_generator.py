...
def create_file(window, name):
    ...
    with open('../trees/' + name + '.json', "w") as json_file:
        json.dump(cur_tree, json_file)
    close_window(window)
...
window = Window(root, title='Treefinity Garden', size=[WIDTH, HEIGHT],
                ...
                canvases=[{'name': 'garden', 'coords': (WIDTH / 64, HEIGHT / 64, WIDTH / 64 + WIDTH * 2 / 3, HEIGHT - HEIGHT / 64), 'bg': 'blue',
...
slider_branch_length_coefficient = create_slider(parent=window.canvas, minimum=0, maximum=1,
                                                 update_callback=update_branch_length_coefficient, pos=(slider_pos_x, HEIGHT / 64 + 60),
...
slider_max_recursion_depth = create_slider(parent=window.canvas, minimum=2, maximum=15,
                                           update_callback=update_max_recursion_depth, pos=(slider_pos_x, HEIGHT / 64 + 120),
...
slider_max_branch_thickness = create_slider(parent=window.canvas, minimum=1, maximum=20,
                                      update_callback=update_max_branch_thickness, pos=(slider_pos_x, HEIGHT / 64 + 300),
...
save_button.place(x=WIDTH - WIDTH / 64 - WIDTH * 13 / 64, y=HEIGHT - HEIGHT / 64 - HEIGHT / 17, width=WIDTH * 5 / 32, height=HEIGHT / 17)

load_button = Button(window.canvas, text='load tree', command=lambda: load_tree(root))
load_button.place(x=WIDTH - WIDTH / 64 - WIDTH * 13 / 64, y=HEIGHT - HEIGHT * 2 / 64 - HEIGHT * 2 / 17, width=WIDTH * 5 / 32, height=HEIGHT / 17)
